...
def create_experiment_workspace(cfg):
    ...
    file_path = cfg.filename.replace("configs/", "").replace(".py", "")
    # exp_name = f"{experiment_index:03d}-F{cfg.num_frames}S{cfg.frame_interval}-{model_name}"
    # exp_name = f"{file_path}-{time_str}"
    exp_name = f"{file_path}"
    # exp_dir = f"{cfg.training.save_path}/{exp_name}"
    exp_dir = os.path.join(cfg.training.save_path, exp_name)
