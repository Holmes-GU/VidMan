...
def save_by_step(
    ...
    booster.save_model(model, os.path.join(save_dir, "model.pt"), shard=False)
    # ema is not boosted, so we don't need to use booster.save_model
    # model_gathering(ema, shape_dict)
    # global_rank = dist.get_rank()
    # if int(global_rank) == 0:
    ...
    booster.save_optimizer(optimizer, os.path.join(save_dir, "optimizer.pt"), shard=False)#, size_per_shard=4096)
    if lr_scheduler is not None:
        booster.save_lr_scheduler(lr_scheduler, os.path.join(save_dir, "lr_scheduler.pt"))
    running_states = {
        "step": step,
        "sample_start_index": step * batch_size,
    }
...
def load(
    booster: Booster, model: nn.Module, ema, optimizer: Optimizer, lr_scheduler: _LRScheduler, load_dir: str
) -> Tuple[int, int, int]:
    booster.load_model(model, os.path.join(load_dir, "model.pt"))
    ...
    booster.load_optimizer(optimizer, os.path.join(load_dir, "optimizer.pt"))
    if lr_scheduler is not None:
        booster.load_lr_scheduler(lr_scheduler, os.path.join(load_dir, "lr_scheduler.pt"))
    running_states = load_json(os.path.join(load_dir, "running_states.json"))
    dist.barrier()
    if hasattr(running_states, "epoch"):
        return running_states["epoch"], running_states["step"], running_states["sample_start_index"]
    else:
        return running_states["step"], running_states["sample_start_index"]
